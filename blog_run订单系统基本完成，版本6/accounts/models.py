...
class User(AbstractUser):
    ...
    def get_integral_detail(self):
        return self.user_integral.all()
...
class UserAddress(models.Model):
    ...
    post_code = models.CharField('邮政编码',max_length=10,null=True,blank=True)
...
class OrderDetail(models.Model):
    ...
    goods_store=models.IntegerField(verbose_name="商品id",null=True,blank=True,default=1)
...
class UserMessage(models.Model):
    user = models.ForeignKey(to=User,on_delete=models.CASCADE,related_name='user_message')
    title = models.CharField('短信息标题',max_length=64)
    content = models.TextField('短信息内容')
    status = models.SmallIntegerField('是否已读',choices=constants.MESSAGE_STATUS_CHOICES,default=constants.MESSAGE_STATUS_UNREAD)
    to_user = models.IntegerField('接收用户id')
    created_at = models.DateTimeField('发送时间',auto_now_add=True)

    class Meta:
        verbose_name_plural = '站内信息'

    def to_user_obj(self):
        return User.objects.get(pk=self.to_user)

    def __str__(self):
        return self.title


class UserMessageAccept(models.Model):
    user = models.ForeignKey(to=User,on_delete=models.CASCADE,related_name='user_message_accept')
    title = models.CharField('短信息标题',max_length=64)
    content = models.TextField('短信息内容')
    status = models.SmallIntegerField('是否已读',choices=constants.MESSAGE_STATUS_CHOICES,default=constants.MESSAGE_STATUS_UNREAD)
    to_user = models.IntegerField('接收用户id')
