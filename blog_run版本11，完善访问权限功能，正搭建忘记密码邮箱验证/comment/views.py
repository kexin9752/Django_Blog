...
class CommentLoad(View):
    ...
    def get(self,request):
        ...
        if not art_uid:
            ...
            level_msg_id = request.GET.get('level_msg_id')
            ...
            if photo_id:
                ...
                print(mall_uid,111)
                comment_count = CommentModel.objects.filter(note_id=note_id).count()
            elif level_msg_id:
                print(mall_uid,111)
                comment_count = CommentModel.objects.filter(level_msg_id=level_msg_id).count()
        else:
            art_obj = Article.objects.get(uid=art_uid)
            comment_count = CommentModel.objects.filter(article=art_obj).count()
    ...
    def post(self,request):
        ...
        if not art_uid:
            ...
            if photo_id:
                ...
                comment_list = CommentModel.objects.filter(level_msg_id=level_msg_id)
            elif note_id:
                comment_list = CommentModel.objects.filter(note_id=note_id)
        else:
            art_obj = Article.objects.get(uid=art_uid)
            comment_list = CommentModel.objects.filter(article=art_obj)  # 获取所有本文章的评论
        print(comment_list)
